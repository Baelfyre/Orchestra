#!/usr/bin/env python3
"""One-attempt-per-slot fail-closed B3 pilot driver."""
...
from scripts.b3_pilot_preflight import FREEZE_PATH, host_preflight, load_json, static_preflight
...
def validate_authorization(auth: dict[str, Any], freeze: dict[str, Any], manifest: dict[str, Any], plan: dict[str, Any]) -> None:
    require(auth.get("schema_version") == "orchestra.b3-pilot-live-authorization.v1", "unsupported authorization schema")
    require(auth.get("live_execution_authorized") is True, "B3 pilot live execution is not authorized")
    expected = {
...
def run_session(auth: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    for entry in plan["entries"]:
        ...
        if result["outcome"]["status"] != "PASS":
            ...
            break
    index = {
        "schema_version": "orchestra.b3-pilot-run-index.v1",
        "planned_runs": 180,
    ...
    summary = {
        "schema_version": "orchestra.b3-pilot-session-summary.v1",
        "status": status,
