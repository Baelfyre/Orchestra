...
PROFILE_SPECS = {
    "backend_persistence": {
        ...
        "required_cases": {
            "backend-persistence-happy-path",
            "backend-contract-mapping-gap",
            "backend-owner-contract-missing",
            "backend-persistence-contradiction",
            "backend-evidence-missing",
            "backend-contract-stale",
    ...
    "cross_module_logic": {
        "stages": (
            ...
            "HANDOFF_PAYLOAD",
            "MODULE_B_DECISION",
            "SHARED_STATE_OR_SIDE_EFFECT",
            "RESULT_PROPAGATION",
            "ERROR_PROPAGATION",
            "FINAL_OBSERVABLE_OUTCOME",
        ),
        "evidence": {
            "input_contract",
            "handoff_contract",
        ...
        "required_cases": {
            "cross-module-happy-path",
            "cross-module-flow-gap",
            "cross-module-owner-contract-missing",
            "cross-module-contradiction",
            "cross-module-evidence-missing",
            "cross-module-contract-stale",
...
def _validate_finding(profile_name, case_id, finding, finding_owner, stages):
    ...
    if (
        ...
        errors.append(
            f"{profile_name}/{case_id}: affected_stages must be a unique non-empty profile stage list"
        )
    refs = finding.get("evidence")
    if not isinstance(refs, list) or not refs or any(not _nonempty(ref) for ref in refs):
        errors.append(
            f"{profile_name}/{case_id}: finding evidence must contain non-empty references"
        )
...
def _validate_workflows(profile_name, profile, stages):
    ...
    for workflow in workflows:
        ...
        if not isinstance(trace, list):
            ...
            continue
        seen_stages = [
            entry.get("stage") if isinstance(entry, dict) else None for entry in trace
        ]
        if tuple(seen_stages) != stages:
            errors.append(
                f"{profile_name}/{workflow_id}: trace must contain profile stages in exact order"
            )
            continue
        for entry in trace:
            stage = entry["stage"]
            if set(entry) != {"stage", "owner", "source_ref", "evidence_ref", "result"}:
                errors.append(
                    f"{profile_name}/{workflow_id}/{stage}: trace entry fields are incomplete or unknown"
                )
                continue
            ...
            for field in ("source_ref", "evidence_ref", "result"):
                if not _nonempty(entry[field]):
                    errors.append(
                        f"{profile_name}/{workflow_id}/{stage}: {field} must be non-empty"
                    )
...
def _validate_profile(profile_name, profile, spec):
    ...
    for case in cases:
        ...
        if (
            ...
            errors.append(
                f"{profile_name}/{case_id}: evidence must contain exactly the profile boolean fields"
            )
            continue
        ...
        if (
            ...
            errors.append(
                f"{profile_name}/{case_id}: expected_reentry must be a unique valid specialist list"
            )

        if workflow_id is not None:
            if workflow_id not in workflow_map:
                errors.append(
                    f"{profile_name}/{case_id}: workflow_id does not reference an executable workflow"
                )
            else:
        ...
        if evidence["executable_workflow"] and not workflow_id:
            errors.append(
                f"{profile_name}/{case_id}: executable evidence requires workflow_id"
            )
        if not evidence["executable_workflow"] and workflow_id is not None:
            errors.append(
                f"{profile_name}/{case_id}: missing executable evidence cannot reference a workflow"
            )
        ...
        if status == "CROSS_LAYER_ALIGNMENT_CONFIRMED":
            if owner is not None or finding is not None or not all(evidence.values()) or reentry:
                errors.append(
                    f"{profile_name}/{case_id}: confirmed alignment requires complete evidence and no finding or re-entry"
                )
        else:
            errors.extend(
                _validate_finding(profile_name, case_id, finding, owner, stages)
            )

        if status == "CROSS_LAYER_EVIDENCE_INSUFFICIENT" and all(evidence.values()):
            errors.append(
                f"{profile_name}/{case_id}: insufficient evidence must identify a missing evidence field"
            )
        if (
            status == "CROSS_LAYER_CONTRACT_STALE"
            and reentry != ["the-tuner", "overseer", "arbiter"]
        ):
            errors.append(
                f"{profile_name}/{case_id}: stale identity must re-enter Tuner, Overseer, and Arbiter"
            )
        if (
            status == "CROSS_LAYER_CONTRADICTION_REVIEW_REQUIRED"
            and "conductor" not in reentry
        ):
            errors.append(
                f"{profile_name}/{case_id}: contradiction must route back through Conductor"
            )
        if status == "SPECIALIST_REENTRY_REQUIRED" and not reentry:
            errors.append(
                f"{profile_name}/{case_id}: re-entry status requires an explicit specialist set"
            )
    ...
    if unreferenced:
        errors.append(
            f"{profile_name}: unreferenced executable workflows {sorted(unreferenced)}"
        )
    return errors
...
def validate_fixtures(data, protocol_bytes=None):
    ...
    if protocol_bytes is not None:
        errors.extend(
            _validate_contract_identity(data.get("contract_identity"), protocol_bytes)
        )

    profiles = data.get("profiles")
    if not isinstance(profiles, dict) or set(profiles) != set(PROFILE_SPECS):
        errors.append(
            "fixtures: profiles must contain exactly backend_persistence and cross_module_logic"
        )
        return errors
...
def validate(repo_root):
    errors = []
    paths = {
        "parent_protocol": repo_root
        / "docs/validation/CROSS_MODULE_LOGIC_AUDIT_PROTOCOL.md",
        "profile_protocol": repo_root
        / "docs/validation/CROSS_LAYER_INTEGRITY_PROFILE_PROTOCOL.md",
        "backend_checklist": repo_root
        / "docs/validation/checklists/BACKEND_PERSISTENCE_INTEGRITY_CHECKLIST.md",
        "module_checklist": repo_root
    ...
    for label, path in paths.items():
        if not path.is_file():
            errors.append(
                f"missing required {label}: {path.relative_to(repo_root).as_posix()}"
            )
    if errors:
        return errors

    parent_protocol = paths["parent_protocol"].read_text(encoding="utf-8")
    profile_bytes = _canonical_text_bytes(paths["profile_protocol"])
    profile_protocol = profile_bytes.decode("utf-8")
    ...
    for status in REQUIRED_STATUSES:
        if status not in parent_protocol or status not in profile_protocol:
            errors.append(f"protocol inheritance: missing deterministic status {status!r}")
    ...
    for token in required_profile_tokens:
        if token not in profile_protocol:
            errors.append(f"profile protocol: missing {token!r}")

    for token in (
        "`COMMIT_OR_ROLLBACK`",
        "Clockwork owns service/repository architecture findings",
        "Chronicler owns schema",
    ):
    ...
    for token in (
        ...
        "Clockwork owns cross-module",
    ):
        if token not in module_checklist:
            errors.append(f"cross-module checklist: missing {token!r}")

    errors.extend(validate_fixtures(load_json(paths["fixtures"]), profile_bytes))
    return errors
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Validate backend-persistence and cross-module logical-flow integrity profiles."
        )
    )
    parser.add_argument(
        "--repo-root",
    ...
    if errors:
        ...
        return 1
    print(
        "[PASS] Cross-layer backend-persistence and cross-module integrity profiles are deterministic and complete."
    )
    return 0
