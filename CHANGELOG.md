...
- Added dry-run-only pre-state, forward-recovery, and branch-retirement contracts: immutable SHA/tree evidence remains canonical, high-risk temporary refs are bounded and non-authorizing, recovery is forward-only through a new validated candidate, and branch classifications can only recommend keep, quarantine, or eligibility for separate deletion authorization.
