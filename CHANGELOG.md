...
- Packages the Required Analysis Compatibility workflow that runs real exact-head Python CodeQL before emitting the historical `Analyze (actions)` and `Analyze (python)` contexts. Issue #331 remains open; this release does not claim direct ruleset identity normalization.
