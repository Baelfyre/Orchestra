...
def _manifest_sha(bundle: Path) -> str:
    with zipfile.ZipFile(bundle) as archive:
        return hashlib.sha256(archive.read("release-manifest.json")).hexdigest()


def _install(bundle: Path, cache: Path, *, expected_tag: str | None = None, allow_rollback: bool = False):
    return compliance_registry.install_bundle(
        bundle,
        cache,
        expected_manifest_sha256=_manifest_sha(bundle),
...
def test_local_install_query_and_pin(tmp_path):
    ...
    _install(_bundle(tmp_path), cache, expected_tag="v1.0.0")
...
def test_local_install_requires_manifest_trust_anchor(tmp_path):
    with pytest.raises(compliance_registry.RegistryError, match="requires an expected release-manifest SHA-256"):
        compliance_registry.install_bundle(_bundle(tmp_path), tmp_path / "cache", expected_tag="v1.0.0")


def test_local_install_rejects_wrong_manifest_trust_anchor(tmp_path):
    with pytest.raises(compliance_registry.RegistryError, match="does not match the expected trust anchor"):
...
def test_older_sequence_requires_explicit_override(tmp_path):
    cache = tmp_path / "cache"
    _install(_bundle(tmp_path, "2.0.0", 2), cache, expected_tag="v2.0.0")
    with pytest.raises(compliance_registry.RegistryError, match="rollback rejected"):
        _install(_bundle(tmp_path, "1.0.0", 1), cache, expected_tag="v1.0.0")
...
def test_release_sequence_collision_is_rejected(tmp_path):
    ...
    with pytest.raises(compliance_registry.RegistryError, match="release sequence collision"):
        _install(_bundle(tmp_path, "1.0.1", 1), cache, expected_tag="v1.0.1")


def test_wrong_canonical_repository_is_rejected(tmp_path):
    bundle = _bundle(tmp_path, canonical_repository="example/attacker")
    with pytest.raises(compliance_registry.RegistryError, match="canonical_repository mismatch"):
        _install(bundle, tmp_path / "cache", expected_tag="v1.0.0")


def test_content_hash_tampering_is_rejected(tmp_path):
    bundle = _bundle(tmp_path, tamper_after_manifest=True)
...
def test_unlisted_bundle_file_is_rejected(tmp_path):
    bundle = _bundle(tmp_path, add_unlisted_file=True)
...
def test_windows_style_archive_path_is_rejected(tmp_path):
...
def test_unsafe_registry_version_is_rejected_before_install_path_use(tmp_path):
    bundle = _bundle(tmp_path, version="../escape", tag="v1.0.0")
...
def test_release_tag_mismatch_is_rejected(tmp_path):
    ...
    with pytest.raises(compliance_registry.RegistryError, match="release tag"):
        _install(bundle, tmp_path / "cache", expected_tag="v9.9.9")
...
def test_non_current_source_state_requires_review(tmp_path):
    cache = tmp_path / "cache"
    _install(_bundle(tmp_path, source_status="CURRENT_WITH_PENDING_CHANGE"), cache, expected_tag="v1.0.0")
...
def test_overdue_source_is_stale(tmp_path):
    cache = tmp_path / "cache"
    _install(_bundle(tmp_path, next_review_due="2000-01-01"), cache, expected_tag="v1.0.0")
    summary = compliance_registry.status(cache)["freshness"]
...
def test_latest_release_must_be_immutable(monkeypatch):
    monkeypatch.setattr(
        compliance_registry,
        "_fetch_json",
        lambda _url: {"draft": False, "prerelease": False, "immutable": False, "tag_name": "v1.0.0"},
    )
    with pytest.raises(compliance_registry.RegistryError, match="not immutable"):
        compliance_registry._latest_release()


def test_update_check_compares_release_tag_not_registry_version(tmp_path, monkeypatch):
    cache = tmp_path / "cache"
    _install(_bundle(tmp_path, version="1.0.0", tag="v1.0.0"), cache, expected_tag="v1.0.0")
