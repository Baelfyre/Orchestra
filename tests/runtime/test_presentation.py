...
def _write_contracts(tmp_path: Path, policy: dict | None = None, vocabulary: dict | None = None) -> None:
    ...
    (destination / "murmurs-policy.v1.json").write_text(json.dumps(policy or _load(POLICY_PATH)), encoding="utf-8")
    (destination / "murmurs-vocabulary.v1.json").write_text(json.dumps(vocabulary or _load(VOCABULARY_PATH)), encoding="utf-8")


def _provenance() -> AuthorityProvenance:
    return AuthorityProvenance(ProvenanceSource.TRUSTED_COMPOSITION, "runtime.root", "v1", "runtime")
...
def test_schema_documents_match_runtime_contract_versions_and_required_events():
    ...
    assert set(policy_schema["properties"]["events"]["required"]) == {item.value for item in PresentationEventKind}
...
def test_invalid_policy_fails_closed_to_explain(tmp_path: Path):
    ...
    decision = decide_presentation(
        PresentationEvent("run-invalid-policy", PresentationEventKind.EXECUTION_HEARTBEAT, 1), tmp_path
    )
    assert decision.disposition is PresentationDisposition.EXPLAIN
...
def test_status_claim_in_vocabulary_fails_closed_to_explain(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="status or completion claims"):
        load_murmurs_vocabulary(tmp_path)
    decision = decide_presentation(
        PresentationEvent("run-invalid-vocabulary", PresentationEventKind.EXECUTION_HEARTBEAT, 2), tmp_path
    )
    assert decision.disposition is PresentationDisposition.EXPLAIN
    assert decision.reason_code == "PRESENTATION_CONTRACT_INVALID"
...
def test_missing_or_invalid_contract_fails_closed_to_explain(tmp_path: Path):
    decision = decide_presentation(
        PresentationEvent("run-missing", PresentationEventKind.EXECUTION_HEARTBEAT, 1), tmp_path
    )
    assert decision.disposition is PresentationDisposition.EXPLAIN
    destination = tmp_path / "machine" / "presentation"
    destination.mkdir(parents=True)
    ...
    second = decide_presentation(
        PresentationEvent("run-invalid-json-shape", PresentationEventKind.EXECUTION_HEARTBEAT, 2), tmp_path
    )
