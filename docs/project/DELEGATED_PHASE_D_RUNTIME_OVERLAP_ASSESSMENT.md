...
Baseline: current green main after clean Delegated Phase C repository-contract replay
...
- `DEFERRED_NOT_RELEASE_BLOCKING` — a valid future capability remains outside the repository implementation required before release preparation. This disposition does not waive an explicit later publication gate such as the replay's installed-host validation requirement.
...
| Context reset / host handoff reliability | Phase C portable host reliability protocol, deterministic repository simulations, live evidence boundary | `DEFERRED_NOT_RELEASE_BLOCKING` for repository implementation only | Repository contracts are complete; actual installed-host evidence remains explicitly pending. It does not block R4-R6 preparation, but it remains a required R7 evidence gate before final publication under the current replay plan. |
...
DEFERRED_NOT_RELEASE_BLOCKING: live installed-host evidence for repository implementation; durable collaboration persistence
```
...
Phase D overlap reconciliation is complete for repository release-planning purposes. Orchestra may proceed to cleanup and `v1.2.0` release-candidate preparation without adding duplicate runtime models.

The remaining Phase C live-host evidence debt must stay visible as `PENDING_LOCAL_HOST_VALIDATION`. Under the current clean replay, R7 installed-host validation remains a publication gate: repository CI alone must not convert that state into `LIVE_VALIDATED`, a release claim, or final publication readiness.
