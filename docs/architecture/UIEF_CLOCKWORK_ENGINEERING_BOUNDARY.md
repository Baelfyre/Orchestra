...
Clockwork consumes an accepted `UIFidelityHandoff` owned by Cloak and binds the translation to the deterministic SHA-256 identity of the full accepted handoff content.
...
Project-native components, tokens, and assets remain preferred when traceable consuming-project evidence binds their identity and they satisfy the accepted handoff. UIEF-5 must not invent project-native evidence.
