...
`AssuranceEvidence` is qualified only when it carries an originating source, authoritative source reference, producer, validator, source/candidate/work-item bindings, freshness and version references, an `AUTHORITATIVE` provenance qualification, an evidence layer, an evidence scope, a claim scope, and an authority reference. The verifier must independently provide all five subject bindings plus `verified_authority_ref`; a record's self-labeled `AUTHORITATIVE` value does not establish authority, and its authority reference must match the verifier-owned value.
...
Evidence scope records what the evidence proves, while claim scope records what the caller is asking to assert. The validator applies the explicit coverage relation `EVIDENCE_SCOPE <= CLAIM_SCOPE`: a narrower scope cannot qualify a broader claim. Required multi-layer tuples are aggregate requirements, so `RUNTIME_VERIFIED` needs both `HTTP` and `RUNTIME`, while `ADVERSARIALLY_VERIFIED` needs both `ADVERSARIAL` and `MUTATION`. Binding and freshness references are checked against the requested source, candidate, work item, version, and freshness values. Missing, stale, mismatched, self-asserted, unverified, or semantically insufficient evidence fails closed.
