...
## Source: SKILL_INDEX.md
    ...
    | `conductor` | Routing and sequencing | ambiguous ownership, cross-domain sequencing, architecture-governance intake, adaptive capacity routing, minimum route composition, access-routing split, workflow orchestration | one obvious specialist owns full task | Routing owner | `SKILL_INDEX.md`; `ROUTING_MAP.md` only for ambiguity; OR-GOV-5 guide for material intake | Routing Plan,Prompts |
