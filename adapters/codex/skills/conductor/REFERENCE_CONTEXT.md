...
## Source: SKILL_INDEX.md
    ...
    | --- | --- | --- | --- | --- | --- | --- |
    | `the-steward` | Business alignment, scope, requirements, SDLC sufficiency | roadmap fit, acceptance criteria, scope boundary, required SDLC artifact sufficiency | legal, licensing, privacy-obligation, IP review | Governance decision owner | `docs/governance/GOVERNANCE_LAYER.md` | Governance Review,Product Intent Contract,Capacity Envelope |
    ...
    | `conductor` | Routing and sequencing | every request classification and specialist selection, ambiguous ownership, cross-domain sequencing, architecture-governance intake, adaptive capacity routing, minimum route composition, access-routing split, workflow orchestration | full orchestration is unnecessary after a clear Conductor-selected single-specialist route | Routing owner | `SKILL_INDEX.md`; `ROUTING_MAP.md` only for ambiguity; OR-GOV-5 guide for material intake | Routing Plan,Prompts |
    | `the-tuner` | Cross-domain contract coordination | multi-domain gap, conflict, governance-contract dependency change, semantic invalidation, or staleness | single-owner work | Coordination only | protocol; governance dependency guide when OR-GOV-6 applies | Collaboration Review,Cross-Layer Contract Packet,Contradiction Report,Re-entry Recommendation |
    | `clockwork` | Architecture, layering, service boundaries, complexity decisions, scale posture, refactor structure | architecture redesign, layering drift, service boundary change, complexity evaluation, scale posture, refactor strategy | straightforward code edit already designed | Technical review owner | none by default | Compact,Full |
    ...
    | `chronicler` | Database and persistence semantics | migration semantics, `MigrationRiskContract`, schema design, ORM behavior, data integrity, normalization | UI review or general QA | Technical review owner | persistence slice only | Caveman,Normalization Output |
