...
1. Validate the accepted `UIFidelityHandoff` ownership and bind `source_revision_or_contract_identity` to `sha256:` plus the repository-native canonical JSON digest of the full accepted handoff.
...
- Preserve project-native component/token/asset precedence only when consuming-project evidence actually binds those identities. Never invent a project-native path, token, icon, provider record, or component merely to satisfy the translation contract.
